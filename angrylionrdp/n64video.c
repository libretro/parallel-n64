...
void render_spans_2cycle_notexel1(int start, int end, int tilenum, int flip)
{
	int tile1, prim_tile, i, j, drinc, dginc, dbinc, dainc, dzinc, dsinc, dtinc, dwinc, dzpix,
		xinc, zb, zbcur;
	UINT8 offx, offy;
	UINT32 blend_en, prewrap, curpixel_cvg, curpixel_cvbit, curpixel_memcvg;
	int dzpixenc, cdith, adith, r, g, b, a, z, s, t, w, sr, sg, sb, sa, sz, ss, st, sw, xstart, xend, xendsc, sss, sst,
		curpixel, x, length, scdiff;
	UINT32 fir, fig, fib;
...
{	
	...
	UINT32 prewrap;
	UINT32 curpixel_cvg, curpixel_cvbit, curpixel_memcvg;
	int dzpixenc, cdith, adith, r, g, b, a, z, sr, sg, sb, sa, sz, xstart, xend, xendsc, curpixel,
		x, length, scdiff;
	UINT32 fir, fig, fib;
...
void render_spans_fill(int start, int end, int flip)
{
	int i, j, fastkillbits, slowkillbits, xinc, xstart, xendsc, prevxstart, curpixel, x, length;
	...
	fastkillbits = other_modes.image_read_en || other_modes.z_compare_en;
	slowkillbits = other_modes.z_update_en && !other_modes.z_source_sel && !fastkillbits;
...
void render_spans_copy(int start, int end, int tilenum, int flip)
{
	int i, j, k, tile1, prim_tile, dsinc, dtinc, dwinc, xinc;
	int s, t, w, ss, st, sw, sss, sst, ssw, diff, fb_index, length, xstart, xendsc;
	int fbadvance, fbptr_advance, copywmask, bytesperpixel, alphamask;
	UINT32 hidword, hidword1, lowdword, lowdword1, fbptr, tempdword, tempbyte, fbendptr;
	...
	hidword1 = 0;
	lowdword1 = 0;
	fbadvance = (fb_size == PIXEL_SIZE_4BIT) ? 8 : 16 >> fb_size;
	fbptr = 0;
	fbptr_advance = flip ? 8 : -8;
	copyqword = 0;
	tempdword = 0;
	tempbyte = 0;
	copywmask = 0;
	alphamask = 0;
	bytesperpixel = (fb_size == PIXEL_SIZE_4BIT) ? 1 : (1 << (fb_size - 1));
	fbendptr = 0;
...
void loading_pipeline(int start, int end, int tilenum, int coord_quad, int ltlut)
{
	int localdebugmode, cnt, i, j, dsinc, dtinc, dswap;
	int s, t, ss, st, xstart, xend, xendsc, sss, sst, ti_index, length;
	int tiadvance, spanadvance, tiptr, tmem_formatting;
	UINT32 tmemidx0, tmemidx1, tmemidx2, tmemidx3;
	UINT32 readval0, readval1, readval2, readval3;
	UINT32 readidx32, bit3fl, hibit;
	UINT64 loadqword;
	UINT16 *tmem16, tempshort;
...
{
	int xfrac, dsdxh, dtdxh, dwdxh, drdxh, dgdxh, dbdxh, dadxh, dzdxh;
	...
	INT32 maxxmx, minxmx, maxxhx, minxhx, xrsc, xlsc, stickybit, yllimit, yhlimit;
	int spix, ycur, ldflag, invaly, length, ylfar;
	int yhclose, allover, allunder, curover, curunder, allinval;
	INT32 clipxlshift, clipxhshift, curcross;
...
{
	int k, sign_dxhdy, do_offset, xfrac, j, xleft, xright, xstart, xend;
	int s, t, w, dsdx, dtdx, dsdy, dtdy, dsde, dtde, tilenum, flip;
	int commandcode, ltlut, coord_quad;
	INT32 yl, ym, yh, xl, xm, xh, dxldy, dxhdy, dxmdy;
	INT32 maxxmx, minxhx;
	int spix, ycur, ylfar, valid_y, length;
	INT32 xrsc, xlsc, stickybit, yllimit, yhlimit;
	...
	commandcode = (lewdata[0] >> 24) & 0x3f;
	ltlut = (commandcode == 0x30);
	coord_quad = ltlut || (commandcode == 0x33);
	flip = 1;
	max_level = 0;
...
{
	int texel1_used_in_cc1, texel0_used_in_cc1, texel0_used_in_cc0, texel1_used_in_cc0;
	int texels_in_cc0, texels_in_cc1;
	int lod_frac_used_in_cc1, lod_frac_used_in_cc0, lodfracused;
	...
	texel1_used_in_cc1 = 0;
	texel0_used_in_cc1 = 0;
	texel0_used_in_cc0 = 0;
	texel1_used_in_cc0 = 0;
	texels_in_cc0 = 0;
	texels_in_cc1 = 0;
	lod_frac_used_in_cc1 = 0;
	lod_frac_used_in_cc0 = 0;
...
static void rdp_load_block(UINT32 w1, UINT32 w2)
{
	...
	int sl, sh, tl, dxt, tilenum, tlclamped;
	INT32 lewdata[10];
...
{
    UINT32 xl, yl, xh, yh, xlint, xhint;
	INT32 ewdata[44];
...
STRICTINLINE void blender_equation_cycle0(int* r, int* g, int* b)
{
	int blend1a, blend2a, blr, blg, blb, sum, mulb;
...
STRICTINLINE UINT32 z_compare(UINT32 zcurpixel, UINT32 sz, UINT16 dzpix, int dzpixenc, UINT32* blend_en, UINT32* prewrap, UINT32* curpixel_cvg, UINT32 curpixel_memcvg)
{
	...
	INT32 rawdzmem;
		UINT32 dznew, dznotshift, farther, dzenc, nearer, max, infront;
		INT32 diff;
		int overflow, cvgcoeff;
...
STRICTINLINE void video_filter16(int* endr, int* endg, int* endb, UINT32 fboffset, UINT32 num, UINT32 hres, UINT32 centercvg)
{
		...
		UINT32 colr, colg, colb, coeff;
    UINT32 idx, leftup, rightup, toleft, toright, leftdown, rightdown;
...
STRICTINLINE void tclod_4x17_to_15(INT32 scurr, INT32 snext, INT32 tcurr, INT32 tnext, INT32 previous, INT32* lod)
{
	...
	dels = SIGN(snext, 17) - SIGN(scurr, 17);
	if (dels & 0x20000)
		dels = ~dels & 0x1ffff;
