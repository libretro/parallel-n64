/* rdp_emit_recip.h -- bit-exact RSP fixed-point reciprocal (1/w).
 * Ported verbatim from cxd4 LLE RSP vu/divide.c (CC0 public domain): the
 * VRCP/VRCPL double-precision reciprocal the microcode uses before the
 * perspective divide. NOTE: DivOut is in the RSP's scaled reciprocal format
 * (1/1.0 -> 0x7fff, ~0.5x), NOT plain s15.16; the microcode applies a
 * compensating x2 when forming screen coords. ISO C89 / MSVC-compatible. */
#ifndef RDP_EMIT_RECIP_H
#define RDP_EMIT_RECIP_H
#include <stdint.h>
static const uint16_t rsp_div_rom[1024] = {
...
/* Double-precision reciprocal of an s15.16 value w. Returns cxd4's DivOut. */
static int32_t rsp_recip_div(int32_t w)
{
    int32_t data = w;
    int32_t addr;
    int shift;
    ...
    shift ^= 31;
    out = (int32_t)((0x40000000UL | ((uint32_t)rsp_div_rom[addr] << 14)) >> shift);
    if (w == 0)
        out = 0x7FFFFFFFl;
    else if (w == -32768)
        out = -0x00010000l;
