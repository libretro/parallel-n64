        ...
        if (bilerp)
        {
                ...
                && state[wid].tile[tilenum].format != FORMAT_YUV)
            {
                ...
                if (state[wid].other_modes.sample_type)
                {
                    ...
                    case 13:
                    case 14:
                        texture_quadro_lerp_ci16_simd(wid, TEX, sss1, sdiff, sst1, tdiff, tilenum, sfrac, tfrac, upper, fcenter, upperrg);
                        return;
                    default:
                        break;
                    }
                ...
                && !convert)
            {
                ...
                case TEXEL_I8:
                    ...
                    return;
                case TEXEL_I4:
                    texture_quadro_lerp_i4_simd(wid, TEX, sss1, sdiff, sst1, tdiff, tilenum, sfrac, tfrac, upper, 0, fcenter);
                    return;
                case TEXEL_IA4:
                    texture_quadro_lerp_i4_simd(wid, TEX, sss1, sdiff, sst1, tdiff, tilenum, sfrac, tfrac, upper, 1, fcenter);
                    return;
                case TEXEL_IA16:
