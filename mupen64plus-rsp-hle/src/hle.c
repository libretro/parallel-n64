...
static ucode_func_t try_audio_task_detection(struct hle_t* hle)
{
    ...
    /* AUDIT-DUMP (local, not for commit): capture each distinct audio ucode
     * with its data and first command lists */
    {
        static uint32_t dumped[16]; static int ndumped = 0; static int nlist[16];
        uint32_t sig = *dram_u32(hle, ucode_data + 0x10) ^ *dram_u32(hle, ucode_data + 0x28) ^ *dram_u32(hle, ucode_data);
        int k, found = -1;
        for (k = 0; k < ndumped; ++k) if (dumped[k] == sig) found = k;
        if (found < 0 && ndumped < 16) {
            char p[256]; FILE *f;
            found = ndumped; dumped[ndumped++] = sig;
            snprintf(p, sizeof(p), "/tmp/ucodes/%08x_text.bin", sig);
            f = fopen(p, "wb");
            if (f) { fwrite((void*)dram_u32(hle, *dmem_u32(hle, TASK_UCODE)), 1, 0x2000, f); fclose(f); }
            snprintf(p, sizeof(p), "/tmp/ucodes/%08x_data.bin", sig);
            f = fopen(p, "wb");
            if (f) { fwrite((void*)dram_u32(hle, ucode_data), 1, 0x1000, f); fclose(f); }
            snprintf(p, sizeof(p), "/tmp/ucodes/%08x_meta.txt", sig);
            f = fopen(p, "w");
            if (f) {
                fprintf(f, "ucode_size=%08x data_size=%08x flags=%08x ucode=%08x data=%08x\n",
                        *dmem_u32(hle, TASK_UCODE_SIZE), *dmem_u32(hle, TASK_UCODE_DATA_SIZE),
                        *dmem_u32(hle, TASK_FLAGS), *dmem_u32(hle, TASK_UCODE), ucode_data);
                fclose(f);
            }
        }
        if (found >= 0 && nlist[found] < 32) {
            char p[256]; FILE *f;
            uint32_t da = *dmem_u32(hle, TASK_DATA_PTR);
            uint32_t sz = *dmem_u32(hle, TASK_DATA_SIZE);
            snprintf(p, sizeof(p), "/tmp/ucodes/%08x_lists.bin", sig);
            f = fopen(p, "ab");
            if (f) {
                /* musyx passes an SFD count, not a byte size; store a
                 * generous fixed window and the raw size field */
                uint32_t wsz = (sz < 0x100) ? 0x3000 : (sz < 0x4000 ? sz : 0x4000);
                fwrite(&wsz, 4, 1, f);
                fwrite(&sz, 4, 1, f);
                fwrite((void*)dram_u32(hle, da), 1, wsz, f);
                fclose(f);
            }
            if (sz < 0x100) {
                /* musyx: also capture the state and sfx blocks and the
                 * active voices' sample sources */
                snprintf(p, sizeof(p), "/tmp/ucodes/%08x_aux.bin", sig);
                f = fopen(p, "ab");
                if (f) {
                    uint32_t k, sp = da;
                    for (k = 0; k < sz && k < 4; ++k) {
                        uint32_t vmask = *dram_u32(hle, sp + 0x4);
                        uint32_t stp = *dram_u32(hle, sp + 0x8) & 0x7fffff;
                        uint32_t sxp = *dram_u32(hle, sp + 0xc) & 0x7fffff;
                        uint32_t vv, ln;
                        if (stp) {
                            ln = 0x400;
                            fwrite(&stp, 4, 1, f); fwrite(&ln, 4, 1, f);
                            fwrite((void*)dram_u32(hle, stp), 1, ln, f);
                        }
                        if (sxp) {
                            ln = 0x2000;
                            fwrite(&sxp, 4, 1, f); fwrite(&ln, 4, 1, f);
                            fwrite((void*)dram_u32(hle, sxp), 1, ln, f);
                        }
                        for (vv = 0; vv < 32; ++vv) {
                            uint32_t vp = sp + 0x10 + vv * 0x50, ci;
                            if (!(vmask & (1u << vv))) continue;
                            for (ci = 0; ci < 2; ++ci) {
                                uint32_t cp = vp + 0x24 + ci * 0xc;
                                uint32_t p1 = *dram_u32(hle, cp) & 0x7fffff;
                                uint32_t p2 = *dram_u32(hle, cp + 4) & 0x7fffff;
                                uint32_t s1 = (*dram_u32(hle, cp + 8) >> 16) & 0xffff;
                                uint32_t s2 = *dram_u32(hle, cp + 8) & 0xffff;
                                if (p1 && s1 && s1 < 0x2000) {
                                    fwrite(&p1, 4, 1, f); fwrite(&s1, 4, 1, f);
                                    fwrite((void*)dram_u32(hle, p1), 1, s1, f);
                                }
                                if (p2 && s2 && s2 < 0x2000) {
                                    fwrite(&p2, 4, 1, f); fwrite(&s2, 4, 1, f);
                                    fwrite((void*)dram_u32(hle, p2), 1, s2, f);
                                }
                            }
                            /* adpcm table */
                            {
                                uint32_t tp = *dram_u32(hle, vp + 0x40) & 0x7fffff;
                                uint32_t ln2 = 0x200;
                                if (tp) {
                                    fwrite(&tp, 4, 1, f); fwrite(&ln2, 4, 1, f);
                                    fwrite((void*)dram_u32(hle, tp), 1, ln2, f);
                                }
                            }
                        }
                        sp += 0x10 + 32 * 0x50;
...
static ucode_func_t try_normal_task_detection(struct hle_t* hle)
{
    ...
    /* AUDIT-DUMP (local): full-scene capture for selected normal tasks */
    {
        unsigned int s256 = sum_bytes((void*)dram_u32(hle, *dmem_u32(hle, TASK_UCODE)), 256);
        unsigned int s1488 = sum_bytes((void*)dram_u32(hle, *dmem_u32(hle, TASK_UCODE)), 1488);
        static unsigned scap[16]; static int nsc = 0;
        int k2, seen = 0;
        if (s1488 == 0x19495 || s1488 == 0x19728) s256 = s1488;
        if (sum == 0x2caa6 || sum == 0x2c85a || sum == 0x130de || sum == 0x278b0)
            s256 = sum;   /* jpeg tasks dispatch on the half-size sum */
        for (k2 = 0; k2 < nsc; ++k2) if (scap[k2] == s256) seen++;
        if (seen < 2 && nsc < 16) {   /* capture every distinct normal task */
            char p2[256]; FILE *f2;
            scap[nsc++] = s256;
            snprintf(p2, sizeof(p2), "/tmp/ucodes/scene_%05x_%d.bin", s256, seen);
            f2 = fopen(p2, "wb");
            if (f2) {
                fwrite((void*)dram_u32(hle, 0), 1, 0x800000, f2);
                fwrite(hle->dmem, 1, 0x1000, f2);
                fclose(f2);
            }
        }
    }
    /* AUDIT-DUMP (local): capture each distinct normal-task ucode */
    {
        static unsigned dumped2[32]; static int nd2 = 0;
        int k, found = -1;
        for (k = 0; k < nd2; ++k) if (dumped2[k] == sum) found = k;
        if (found < 0 && nd2 < 32) {
            char p[256]; FILE *f;
            dumped2[nd2++] = sum;
            snprintf(p, sizeof(p), "/tmp/ucodes/task_%05x_text.bin", sum);
            f = fopen(p, "wb");
            if (f) { fwrite((void*)dram_u32(hle, *dmem_u32(hle, TASK_UCODE)), 1, 0x1000, f); fclose(f); }
            snprintf(p, sizeof(p), "/tmp/ucodes/task_%05x_data.bin", sum);
            f = fopen(p, "wb");
            if (f) { fwrite((void*)dram_u32(hle, *dmem_u32(hle, TASK_UCODE_DATA)), 1, 0x800, f); fclose(f); }
        }
    }
    {
