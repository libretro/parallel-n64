...
struct GBIInfo
{
	...
	uint32_t PCStackSize;

	void init();
	void destroy();
	void loadMicrocode(uint32_t uc_start, uint32_t uc_dstart, uint16_t uc_dsize);
	uint32_t getMicrocodeType() const {return m_pCurrent != NULL ? m_pCurrent->type : NONE;}
...
private:
	...
	bool _makeExistingMicrocodeCurrent(uint32_t uc_start, uint32_t uc_dstart, uint32_t uc_dsize);
