...
static void uc9_draw_object (wxUint8 * addr, wxUint32 type)
{
  ...
  {
    VERTEX &v = vtx[i];
    v.sx = zSortRdp.scale_x * ((wxInt16*)addr)[0^1];
    v.sy = zSortRdp.scale_y * ((wxInt16*)addr)[1^1];
    v.sz = 1.0f;
    v.r = addr[4^3];
    v.g = addr[5^3];
    v.b = addr[6^3];
    ...
    if (textured)
    {
      v.ou = ((wxInt16*)addr)[4^1];
      v.ov = ((wxInt16*)addr)[5^1];
...
static wxUint32 uc9_load_object (wxUint32 zHeader, wxUint32 * rdpcmds)
{
  ...
  wxUint8 * addr = GFX_PTR.RDRAM + (zHeader&0xFFFFFFF8);
  switch (type) {
    case 1: //sh tri
    case 3: //sh quad
      {
...
static void uc9_fmlight ()
{
  ...
  rdp.light[rdp.num_lights].r = (float)(((wxUint8*)GFX_PTR.DMEM)[(a+0)^3]) / 255.0f;
  rdp.light[rdp.num_lights].g = (float)(((wxUint8*)GFX_PTR.DMEM)[(a+1)^3]) / 255.0f;
  rdp.light[rdp.num_lights].b = (float)(((wxUint8*)GFX_PTR.DMEM)[(a+2)^3]) / 255.0f;
  ...
  {
    rdp.light[i].r = (float)(((wxUint8*)GFX_PTR.DMEM)[(a+0)^3]) / 255.0f;
    rdp.light[i].g = (float)(((wxUint8*)GFX_PTR.DMEM)[(a+1)^3]) / 255.0f;
    rdp.light[i].b = (float)(((wxUint8*)GFX_PTR.DMEM)[(a+2)^3]) / 255.0f;
    rdp.light[i].a = 1.0f;
    rdp.light[i].dir_x = (float)(((wxInt8*)GFX_PTR.DMEM)[(a+8)^3]) / 127.0f;
    rdp.light[i].dir_y = (float)(((wxInt8*)GFX_PTR.DMEM)[(a+9)^3]) / 127.0f;
    rdp.light[i].dir_z = (float)(((wxInt8*)GFX_PTR.DMEM)[(a+10)^3]) / 127.0f;
...
//    TransformVector(&rdp.light[i].dir_x, rdp.light_vector[i], *m);
  ...
  for (i = 0; i < 2; i++)
  {
    float dir_x = (float)(((wxInt8*)GFX_PTR.DMEM)[(a+8)^3]) / 127.0f;
    float dir_y = (float)(((wxInt8*)GFX_PTR.DMEM)[(a+9)^3]) / 127.0f;
    float dir_z = (float)(((wxInt8*)GFX_PTR.DMEM)[(a+10)^3]) / 127.0f;
...
static void uc9_light ()
{
  ...
  {
    v.vec[0] = ((wxInt8*)GFX_PTR.DMEM)[(nsrs++)^3];
    v.vec[1] = ((wxInt8*)GFX_PTR.DMEM)[(nsrs++)^3];
    v.vec[2] = ((wxInt8*)GFX_PTR.DMEM)[(nsrs++)^3];
    calc_sphere (&v);
//    calc_linear (&v);
    ...
    if (use_material)
    {
      v.r = (wxUint8)(((wxUint32)v.r * GFX_PTR.DMEM[(csrs++)^3])>>8);
      v.g = (wxUint8)(((wxUint32)v.g * GFX_PTR.DMEM[(csrs++)^3])>>8);
      v.b = (wxUint8)(((wxUint32)v.b * GFX_PTR.DMEM[(csrs++)^3])>>8);
      v.a = GFX_PTR.DMEM[(csrs++)^3];
    }
    GFX_PTR.DMEM[(cdest++)^3] = v.r;
    GFX_PTR.DMEM[(cdest++)^3] = v.g;
    GFX_PTR.DMEM[(cdest++)^3] = v.b;
    GFX_PTR.DMEM[(cdest++)^3] = v.a;
    ((wxInt16*)GFX_PTR.DMEM)[(tdest++)^1] = (wxInt16)v.ou;
    ((wxInt16*)GFX_PTR.DMEM)[(tdest++)^1] = (wxInt16)v.ov;
  }
}
...
static void uc9_mult_mpmtx ()
{
  ...
  wxInt16 * saddr = (wxInt16*)(GFX_PTR.DMEM+src);
  zSortVDest * daddr = (zSortVDest*)(GFX_PTR.DMEM+dst);
  int idx = 0;
  zSortVDest v;
  ...
  for (int i = 0; i < num; i++)
  {
    wxInt16 sx   = saddr[(idx++)^1];
    wxInt16 sy   = saddr[(idx++)^1];
    wxInt16 sz   = saddr[(idx++)^1];
    ...
    v.sx = (wxInt16)(zSortRdp.view_trans[0] + x / w * zSortRdp.view_scale[0]);
    v.sy = (wxInt16)(zSortRdp.view_trans[1] + y / w * zSortRdp.view_scale[1]);
    ...
    daddr[i] = v;
    //memcpy(GFX_PTR.DMEM+dst+sizeof(zSortVDest)*i, &v, sizeof(zSortVDest));
...
void uc9_movemem ()
{
  ...
  case 0: //save/load
    if (flag == 0)
    {
      ...
      memcpy(GFX_PTR.DMEM + dmem_addr, GFX_PTR.RDRAM + addr, len);
    }
    else
    {
      int dmem_addr = (idx<<3) + ofs;
      FRDP ("Load from DMEM. %08lx -> %08lx\n", dmem_addr, addr);
      memcpy(GFX_PTR.RDRAM + addr, GFX_PTR.DMEM + dmem_addr, len);
    }
    break;
  ...
  case 12:   // VIEWPORT
    {
      ...
      rdp.fog_multiplier = ((wxInt16*)GFX_PTR.RDRAM)[(a+3)^1];
      ...
      rdp.fog_offset = ((wxInt16*)GFX_PTR.RDRAM)[(a+7)^1];
