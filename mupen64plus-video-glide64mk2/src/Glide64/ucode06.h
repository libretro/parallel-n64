...
{
  uint16_t * src = (uint16_t*)(GFX_PTR.RDRAM+d.imagePtr);
...
void DrawImage (DRAWIMAGE & d)
{
  ...
  {
    uint16_t width = (uint16_t)(*GFX_PTR.VI_WIDTH_REG & 0xFFF);
...
static void uc6_read_background_data (DRAWIMAGE & d, bool bReadScale)
{
  ...
  d.imageX      = (((uint16_t *)GFX_PTR.RDRAM)[(addr+0)^1] >> 5);   // 0
  d.imageW      = (((uint16_t *)GFX_PTR.RDRAM)[(addr+1)^1] >> 2);   // 1
  d.frameX      = ((int16_t*)GFX_PTR.RDRAM)[(addr+2)^1] / 4.0f;       // 2
  d.frameW      = ((uint16_t *)GFX_PTR.RDRAM)[(addr+3)^1] >> 2;             // 3

  d.imageY      = (((uint16_t *)GFX_PTR.RDRAM)[(addr+4)^1] >> 5);   // 4
  d.imageH      = (((uint16_t *)GFX_PTR.RDRAM)[(addr+5)^1] >> 2);   // 5
  d.frameY      = ((int16_t*)GFX_PTR.RDRAM)[(addr+6)^1] / 4.0f;       // 6
  d.frameH      = ((uint16_t *)GFX_PTR.RDRAM)[(addr+7)^1] >> 2;             // 7

  d.imagePtr    = segoffset(((uint32_t*)GFX_PTR.RDRAM)[(addr+8)>>1]);       // 8,9
  d.imageFmt    = ((uint8_t *)GFX_PTR.RDRAM)[(((addr+11)<<1)+0)^3]; // 11
  d.imageSiz    = ((uint8_t *)GFX_PTR.RDRAM)[(((addr+11)<<1)+1)^3]; // |
  d.imagePal    = ((uint16_t *)GFX_PTR.RDRAM)[(addr+12)^1]; // 12
  uint16_t imageFlip = ((uint16_t *)GFX_PTR.RDRAM)[(addr+13)^1];    // 13;
  d.flipX       = (uint8_t)imageFlip&0x01;

  if (bReadScale)
  {
    d.scaleX      = ((int16_t *)GFX_PTR.RDRAM)[(addr+14)^1] / 1024.0f;  // 14
    d.scaleY      = ((int16_t *)GFX_PTR.RDRAM)[(addr+15)^1] / 1024.0f;  // 15
  }
  else
...
static void uc6_read_object_data (DRAWOBJECT & d)
{
  ...
  d.objX            = ((int16_t*)GFX_PTR.RDRAM)[(addr+0)^1] / 4.0f;               // 0
  d.scaleW  = ((uint16_t *)GFX_PTR.RDRAM)[(addr+1)^1] / 1024.0f;        // 1
  d.imageW  = ((int16_t*)GFX_PTR.RDRAM)[(addr+2)^1] >> 5;                 // 2, 3 is padding
  d.objY            = ((int16_t*)GFX_PTR.RDRAM)[(addr+4)^1] / 4.0f;               // 4
  d.scaleH  = ((uint16_t *)GFX_PTR.RDRAM)[(addr+5)^1] / 1024.0f;        // 5
  d.imageH  = ((int16_t*)GFX_PTR.RDRAM)[(addr+6)^1] >> 5;                 // 6, 7 is padding

  d.imageStride = ((uint16_t *)GFX_PTR.RDRAM)[(addr+8)^1];                  // 8
  d.imageAdrs           = ((uint16_t *)GFX_PTR.RDRAM)[(addr+9)^1];                  // 9
  d.imageFmt             = ((uint8_t *)GFX_PTR.RDRAM)[(((addr+10)<<1)+0)^3]; // 10
  d.imageSiz             = ((uint8_t *)GFX_PTR.RDRAM)[(((addr+10)<<1)+1)^3]; // |
  d.imagePal             = ((uint8_t *)GFX_PTR.RDRAM)[(((addr+10)<<1)+2)^3]; // 11
  d.imageFlags   = ((uint8_t *)GFX_PTR.RDRAM)[(((addr+10)<<1)+3)^3]; // |
...
static void uc6_obj_movemem ()
{
  ...
  if (index == 0) {     // movemem matrix
    ...
    mat_2d.X = ((int16_t*)GFX_PTR.RDRAM)[(addr+8)^1] / 4.0f;
    mat_2d.Y = ((int16_t*)GFX_PTR.RDRAM)[(addr+9)^1] / 4.0f;
    mat_2d.BaseScaleX = ((uint16_t*)GFX_PTR.RDRAM)[(addr+10)^1] / 1024.0f;
    mat_2d.BaseScaleY = ((uint16_t*)GFX_PTR.RDRAM)[(addr+11)^1] / 1024.0f;
    ...
    mat_2d.X = ((int16_t*)GFX_PTR.RDRAM)[(addr+0)^1] / 4.0f;
    mat_2d.Y = ((int16_t*)GFX_PTR.RDRAM)[(addr+1)^1] / 4.0f;
    mat_2d.BaseScaleX = ((uint16_t*)GFX_PTR.RDRAM)[(addr+2)^1] / 1024.0f;
    mat_2d.BaseScaleY = ((uint16_t*)GFX_PTR.RDRAM)[(addr+3)^1] / 1024.0f;
...
static void uc6_DrawYUVImageToFrameBuffer(uint16_t ul_x, uint16_t ul_y, uint16_t lr_x, uint16_t lr_y)
{
  ...
  uint32_t * mb = (uint32_t*)(GFX_PTR.RDRAM+rdp.timg.addr); //pointer to the first macro block
  uint16_t * dst = (uint16_t*)(GFX_PTR.RDRAM+rdp.cimg);
...
static void uc6_obj_loadtxtr ()
{
  ...
  uint32_t addr = segoffset(rdp.cmd1) >> 1;
  uint32_t type = ((uint32_t*)GFX_PTR.RDRAM)[(addr + 0) >> 1];                      // 0, 1

  if (type == 0x00000030) {     // TLUT
    uint32_t image              = segoffset(((uint32_t*)GFX_PTR.RDRAM)[(addr + 2) >> 1]);   // 2, 3
    uint16_t  phead             = ((uint16_t *)GFX_PTR.RDRAM)[(addr + 4) ^ 1] - 256;        // 4
    uint16_t  pnum              = ((uint16_t *)GFX_PTR.RDRAM)[(addr + 5) ^ 1] + 1;          // 5
    ...
    uint32_t image              = segoffset(((uint32_t*)GFX_PTR.RDRAM)[(addr + 2) >> 1]);   // 2, 3
    uint16_t  tmem              = ((uint16_t *)GFX_PTR.RDRAM)[(addr + 4) ^ 1];      // 4
    uint16_t  tsize             = ((uint16_t *)GFX_PTR.RDRAM)[(addr + 5) ^ 1];      // 5
    uint16_t  tline             = ((uint16_t *)GFX_PTR.RDRAM)[(addr + 6) ^ 1];      // 6
  ...
  else if (type == 0x00fc1034)
  {
    uint32_t image              = segoffset(((uint32_t*)GFX_PTR.RDRAM)[(addr + 2) >> 1]);   // 2, 3
    uint16_t  tmem              = ((uint16_t *)GFX_PTR.RDRAM)[(addr + 4) ^ 1];      // 4
    uint16_t  twidth    = ((uint16_t *)GFX_PTR.RDRAM)[(addr + 5) ^ 1];      // 5
    uint16_t  theight   = ((uint16_t *)GFX_PTR.RDRAM)[(addr + 6) ^ 1];      // 6
...
void uc6_sprite2d ()
{
  ...
  uint32_t cmd0 = ((uint32_t*)GFX_PTR.RDRAM)[a>>2]; //check next command
  if ( (cmd0>>24) != 0xBE )
    return;
  ...
  d.imagePtr    = segoffset(((uint32_t*)GFX_PTR.RDRAM)[(addr+0)>>1]);       // 0,1
  uint16_t stride = (((uint16_t *)GFX_PTR.RDRAM)[(addr+4)^1]);      // 4
  d.imageW      = (((uint16_t *)GFX_PTR.RDRAM)[(addr+5)^1]);        // 5
  d.imageH      = (((uint16_t *)GFX_PTR.RDRAM)[(addr+6)^1]);        // 6
  d.imageFmt    = ((uint8_t *)GFX_PTR.RDRAM)[(((addr+7)<<1)+0)^3];  // 7
  d.imageSiz    = ((uint8_t *)GFX_PTR.RDRAM)[(((addr+7)<<1)+1)^3];  // |
  d.imagePal    = 0;
  d.imageX      = (((uint16_t *)GFX_PTR.RDRAM)[(addr+8)^1]);        // 8
  d.imageY      = (((uint16_t *)GFX_PTR.RDRAM)[(addr+9)^1]);        // 9
  uint32_t tlut         = ((uint32_t*)GFX_PTR.RDRAM)[(addr + 2) >> 1];      // 2, 3
  ...
  cmd0 = ((uint32_t*)GFX_PTR.RDRAM)[a>>2]; //check next command
  while (1)
  {
    if ( (cmd0>>24) == 0xBE )
    {
      uint32_t cmd1 = ((uint32_t*)GFX_PTR.RDRAM)[(a>>2)+1];
