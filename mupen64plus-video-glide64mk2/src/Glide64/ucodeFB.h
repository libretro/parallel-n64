...
static void fb_bg_copy ()
{
  ...
  uint8_t imageFmt	= ((uint8_t *)GFX_PTR.RDRAM)[(((addr+11)<<1)+0)^3];
  uint8_t imageSiz	= ((uint8_t *)GFX_PTR.RDRAM)[(((addr+11)<<1)+1)^3];
  uint32_t imagePtr	= segoffset(((uint32_t*)GFX_PTR.RDRAM)[(addr+8)>>1]);
  ...
  if (status == ci_main)
  {
    uint16_t frameW	= ((uint16_t *)GFX_PTR.RDRAM)[(addr+3)^1] >> 2;
    uint16_t frameH	= ((uint16_t *)GFX_PTR.RDRAM)[(addr+7)^1] >> 2;
