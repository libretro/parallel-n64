...
static void uc8_vertex ()
{
	...
	for (i=0; i < (n<<4); i+=16)
	{
		...
		{
			uint32_t shift = v0 << 1;
			v->vec[0] = ((int8_t*)GFX_PTR.RDRAM)[(uc8_normale_addr + (i>>3) + shift + 0)^3];
			v->vec[1] = ((int8_t*)GFX_PTR.RDRAM)[(uc8_normale_addr + (i>>3) + shift + 1)^3];
...
static void uc8_movemem ()
{
	...
	case 10:  // LIGHT
		{
			...
			// **
			uint32_t a = addr >> 1;
			rdp.light[n].x = (float)(((int16_t*)GFX_PTR.RDRAM)[(a+16)^1]);
			rdp.light[n].y = (float)(((int16_t*)GFX_PTR.RDRAM)[(a+17)^1]);
			rdp.light[n].z = (float)(((int16_t*)GFX_PTR.RDRAM)[(a+18)^1]);
			rdp.light[n].w = (float)(((int16_t*)GFX_PTR.RDRAM)[(a+19)^1]);
			...
			for (int t=0; t < 24; t++)
			{
				FRDP ("light[%d] = 0x%04lx \n", t, ((uint16_t*)GFX_PTR.RDRAM)[(a+t)^1]);
			}
#endif
		}
		break;
	...
	case 14: //Normales
      ...
      int i;
			for (i = 0; i < 32; i++)
			{
				int8_t x = ((int8_t*)GFX_PTR.RDRAM)[uc8_normale_addr + ((i<<1) + 0)^3];
				int8_t y = ((int8_t*)GFX_PTR.RDRAM)[uc8_normale_addr + ((i<<1) + 1)^3];
				FRDP("#%d x = %d, y = %d\n", i, x, y);
			}
			uint32_t a = uc8_normale_addr >> 1;
			for (i = 0; i < 32; i++)
			{
				FRDP ("n[%d] = 0x%04lx \n", i, ((uint16_t*)GFX_PTR.RDRAM)[(a+i)^1]);
			}
#endif
		}
		break;
