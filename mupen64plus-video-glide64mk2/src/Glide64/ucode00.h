...
void load_matrix (float m[4][4], wxUint32 addr)
{
  ...
  addr >>= 1;
  wxUint16 * src = (wxUint16*)GFX_PTR.RDRAM;
...
static void uc0_movemem()
{
  ...
  case 0x94:
    ...
    // Get the data
    rdp.light[i].r = (float)(((wxUint8*)GFX_PTR.RDRAM)[(a+0)^3]) / 255.0f;
    rdp.light[i].g = (float)(((wxUint8*)GFX_PTR.RDRAM)[(a+1)^3]) / 255.0f;
    rdp.light[i].b = (float)(((wxUint8*)GFX_PTR.RDRAM)[(a+2)^3]) / 255.0f;
    rdp.light[i].a = 1.0f;
