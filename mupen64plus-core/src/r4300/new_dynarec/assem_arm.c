  ...
  else if(ptr[3]==0x72)
  {
    ...
    else if((target-(u_int)ptr2-8)<4096&&!((target-(u_int)ptr2-8)&15))
    {
      ...
      *ptr2=(*ptr2&0xFFFFF000)|((target-(u_int)ptr2-8)>>4)|0xE00;
       ...
       *ptr2=(0x7A000000)|(((target-(u_int)ptr2-8)<<6)>>8);
  }
  else
  {
...
static int verify_dirty(void *addr)
{
  ...
  if(verifier==(u_int)verify_code_vm||verifier==(u_int)verify_code_ds) {
    ...
    {
      if((memory_map[++page]<<2)!=(map_value<<2))
         return 0;
    }
...
#endif
#if 0
  printf("verify_dirty: %x %x %x",source,copy,len);
#endif
...
static void alloc_reg(struct regstat *cur,int i,signed char reg)
{
...
#if 0
  printf("eax=%d ecx=%d edx=%d ebx=%d ebp=%d esi=%d edi=%d",cur->regmap[0],cur->regmap[1],cur->regmap[2],cur->regmap[3],cur->regmap[5],cur->regmap[6],cur->regmap[7]);
  printf("hsn(%x): %d %d %d %d %d %d %d",start+i*4,hsn[cur->regmap[0]&63],hsn[cur->regmap[1]&63],hsn[cur->regmap[2]&63],hsn[cur->regmap[3]&63],hsn[cur->regmap[5]&63],hsn[cur->regmap[6]&63],hsn[cur->regmap[7]&63]);
#endif
  if(i>0)
  {
    ...
    for(j=10;j>=3;j--)
    {
      /* Allocate preferred register if available */
  ...
  printf("This shouldn't happen (alloc_reg)");
  exit(1);
}
...
static void multdiv_assemble_arm(int i,struct regstat *i_regs)
{
  ...
  if(rs1[i]&&rs2[i])
  {
    ...
    else // 64-bit
    {
      ...
      for(hr=0;hr<HOST_REGS;hr++)
      {
        if(i_regs->regmap[hr]>=0 && (i_regs->regmap[hr]&62)!=HIREG)
           reglist|=1<<hr;
      }
#endif
        signed char d1h=get_reg(i_regs->regmap,rs1[i]|64);
...
static void wb_sx(signed char pre[],signed char entry[],uint64_t dirty,
      uint64_t is32_pre,uint64_t is32,uint64_t u,uint64_t uu)
{
