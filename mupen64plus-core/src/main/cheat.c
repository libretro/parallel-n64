...
void cheat_apply_cheats(int entry)
{
         ...
         ) && entry == ENTRY_VI)
   {
    ...
    {
       /* These are GameShark codes: the D-type line is a conditional that gates
        * the write that follows it -- the write must only run when the condition
        * holds. execute_cheat() returns that condition for D-codes, so honor it.
        * Issuing the 8-bit write unconditionally (ignoring the return) makes it
        * fire on every VI. In the Professor Oak report / preview state the heap
        * region these addresses point at is re-used by a UI sprite descriptor,
        * and the stray write zeroes the sprite's bmHreal field. The draw routine
        * then issues gDPLoadBlock with tex_width*0-1 == 0xFFFFFFFF, over-reading
        * TMEM and producing the diagonal blue/white static on the report preview
        * (libretro/parallel-n64#669). */
       if (sl(ROM_HEADER.CRC1) == 0xCA12B547 && sl(ROM_HEADER.CRC2) == 0x71FA4EE4) {
          // Pokemon Snap (U)
          if (execute_cheat(0xD1382D1C, 0x0002, NULL))
          ...
          // Pokemon Snap (J) (V1.0)
          if (execute_cheat(0xD136D22C, 0x802A, NULL))
          ...
          // Pokemon Snap (G)
          if (execute_cheat(0xD1381BDC, 0x802C, NULL))
             execute_cheat(0x80381BCF, 0x0000, NULL);
       }
       else {
          // Pokemon Snap (E) + (F) + (I) + (S)
          if (execute_cheat(0xD1381BFC, 0x802C, NULL))
