...
static void dma_sp_do(struct rsp_core* sp, unsigned memaddr, unsigned dramaddr,
      unsigned length, unsigned count, unsigned skip)
{
    unsigned int i,j;
   ...
   uint32_t reg        = RSP_REG(address);
    ...
    switch(reg)
    {
       case SP_RD_LEN_REG:
          ...
          count    = ((l >> 12) & 0xff) + 1;
          skip     = ((l >> 20) & 0xfff);
          memaddr  = sp->regs[SP_MEM_ADDR_REG] & 0xfff;
          dramaddr = sp->regs[SP_DRAM_ADDR_REG] & 0xffffff;

          dma_sp_do(sp, memaddr, dramaddr, length, count, skip);
          break;
       case SP_WR_LEN_REG:
          ...
          memaddr  = sp->regs[SP_MEM_ADDR_REG] & 0xfff;
          dramaddr = sp->regs[SP_DRAM_ADDR_REG] & 0xffffff;
          dma_sp_do(sp, dramaddr, memaddr, length, count, skip);
          break;
