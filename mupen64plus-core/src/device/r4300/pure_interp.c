...
{
   /* Re-enterable per-frame: only seed PC on the very first entry; on
    * subsequent slices resume from the current interp_PC. frame_break
    * (set by retro_return at the VI interrupt) breaks the loop and returns
    * WITHOUT tearing anything down, so the next call continues the game. */
   static int l_pi_started = 0;
   if (!l_pi_started)
   {
      l_pi_started = 1;
      *r4300_stop(r4300) = 0;
