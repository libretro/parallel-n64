...
#pragma push_macro("hi")
#pragma push_macro("lo")
#undef pcaddr
...
static void print_debug_info(u_int vaddr)
{
  ...
  DebugMessage(M64MSG_VERBOSE, "target addr:%.8x\n",vaddr);
  //DebugMessage(M64MSG_VERBOSE, "rdram checksum:%x\n",rdram_checksum());
  DebugMessage(M64MSG_VERBOSE, "gpr checksum:%x\n",gpr_sum);
  DebugMessage(M64MSG_VERBOSE, "fpr checksum:%x\n",fpr_sum);
  DebugMessage(M64MSG_VERBOSE, "cop0_checksum:%x\n",cop0_sum);
  /*int i;
  if(gpr_sum!=0){
    for(i=0;i<32;i++)
      DebugMessage(M64MSG_VERBOSE, "r%d:%.8x%.8x\n",i,((int *)(reg+i))[1],((int *)(reg+i))[0]);
  }
  if(fpr_sum!=0){
    for(i=0;i<32;i++)
      DebugMessage(M64MSG_VERBOSE, "f%d:%.8x%.8x\n",i,((int*)reg_cop1_simple[i])[1],*((int*)reg_cop1_simple[i]));
  }
  if(cop0_sum!=0){
    for(i=0;i<32;i++)
      DebugMessage(M64MSG_VERBOSE, "cop0%d:%.8x\n",i,g_cp0_regs[i]);
  }*/
  DebugMessage(M64MSG_VERBOSE, "hi:%.8x%.8x, lo:%.8x%.8x\n",(int)(hi>>32),(int)hi,(int)(lo>>32),(int)lo);
  DebugMessage(M64MSG_VERBOSE, "FCR31:%.8x, FCR0:%.8x\n",FCR31,FCR0);
  DebugMessage(M64MSG_VERBOSE, "count:%.8x, next:%.8x\n\n",g_cp0_regs[CP0_COUNT_REG],next_interrupt);
}
...
static void print_pc(int vaddr)
{
  ...
  DebugMessage(M64MSG_VERBOSE, "PC:%.8x\n",vaddr);
...
void *get_addr(u_int vaddr)
{
  ...
  //DebugMessage(M64MSG_VERBOSE, "TRACE: count=%d next=%d (get_addr match %x: %x)",g_cp0_regs[CP0_COUNT_REG],next_interrupt,vaddr,(intptr_t)head->addr);
  ...
  while(head!=NULL) {
    if(head->vaddr==vaddr&&head->reg32==0) {
      //DebugMessage(M64MSG_VERBOSE, "TRACE: count=%d next=%d (get_addr match dirty %x: %x)",g_cp0_regs[CP0_COUNT_REG],next_interrupt,vaddr,(intptr_t)head->addr);
...
void *get_addr_32(u_int vaddr,u_int flags)
{
  ...
  while(head!=NULL) {
    if(head->vaddr==vaddr&&(head->reg32&flags)==0) {
      //DebugMessage(M64MSG_VERBOSE, "TRACE: count=%d next=%d (get_addr_32 match %x: %x)",g_cp0_regs[CP0_COUNT_REG],next_interrupt,vaddr,(intptr_t)head->addr);
      if(head->reg32==0) {
        uintptr_t *ht_bin=hash_table[((vaddr>>16)^vaddr)&0xFFFF];
        if(ht_bin[0]==-1) {
          ht_bin[1]=(intptr_t)head->addr;
  ...
  while(head!=NULL) {
    if(head->vaddr==vaddr&&(head->reg32&flags)==0) {
      //DebugMessage(M64MSG_VERBOSE, "TRACE: count=%d next=%d (get_addr_32 match dirty %x: %x)",g_cp0_regs[CP0_COUNT_REG],next_interrupt,vaddr,(intptr_t)head->addr);
      // Don't restore blocks which are about to expire from the cache
      if((((uintptr_t)head->addr-(uintptr_t)out)<<(32-TARGET_SIZE_2))>0x60000000+(MAX_OUTPUT_BLOCK_SIZE<<(32-TARGET_SIZE_2))) {
        if(verify_dirty(head->addr)) {
          //DebugMessage(M64MSG_VERBOSE, "restore candidate: %x (%d) d=%d",vaddr,page,invalid_code[vaddr>>12]);
...
static void do_ccstub(int n)
{
  ...
  emit_call((intptr_t)gen_interrupt);
  emit_readword((intptr_t)&g_cp0_regs[CP0_COUNT_REG],HOST_CCREG);
  emit_readword((intptr_t)&next_interrupt,EAX);
