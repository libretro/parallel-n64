...
void *get_addr(u_int vaddr)
{
  ...
  //DebugMessage(M64MSG_VERBOSE, "TRACE: count=%d next=%d (get_addr %x,page %d)",g_cp0_regs[CP0_COUNT_REG],next_interrupt,vaddr,page);
  head=jump_in[page];
  while(head!=NULL) {
    if(head->vaddr==vaddr&&head->reg_sv_flags ==0) {
  //DebugMessage(M64MSG_VERBOSE, "TRACE: count=%d next=%d (get_addr match %x: %x)",g_cp0_regs[CP0_COUNT_REG],next_interrupt,vaddr,(int)head->addr);
  ...
  //DebugMessage(M64MSG_VERBOSE, "TRACE: count=%d next=%d (get_addr no-match %x)",g_cp0_regs[CP0_COUNT_REG],next_interrupt,vaddr);
...
void *get_addr_ht(u_int vaddr)
{
  //DebugMessage(M64MSG_VERBOSE, "TRACE: count=%d next=%d (get_addr_ht %x)",g_cp0_regs[CP0_COUNT_REG],next_interrupt,vaddr);
  u_int *ht_bin=hash_table[((vaddr>>16)^vaddr)&0xFFFF];
...
void *get_addr_32(u_int vaddr,u_int flags)
{
  //DebugMessage(M64MSG_VERBOSE, "TRACE: count=%d next=%d (get_addr_32 %x,flags %x)",g_cp0_regs[CP0_COUNT_REG],next_interrupt,vaddr,flags);
  u_int *ht_bin=hash_table[((vaddr>>16)^vaddr)&0xFFFF];
  if(ht_bin[0]==vaddr) return (void *)ht_bin[1];
  if(ht_bin[2]==vaddr) return (void *)ht_bin[3];
  u_int page=(vaddr^0x80000000)>>12;
  ...
  while(head!=NULL) {
    if(head->vaddr==vaddr&&(head->reg_sv_flags &flags)==0) {
      //DebugMessage(M64MSG_VERBOSE, "TRACE: count=%d next=%d (get_addr_32 match %x: %x)",g_cp0_regs[CP0_COUNT_REG],next_interrupt,vaddr,(int)head->addr);
  ...
  while(head!=NULL) {
    if(head->vaddr==vaddr&&(head->reg_sv_flags &flags)==0) {
      //DebugMessage(M64MSG_VERBOSE, "TRACE: count=%d next=%d (get_addr_32 match dirty %x: %x)",g_cp0_regs[CP0_COUNT_REG],next_interrupt,vaddr,(int)head->addr);
      // Don't restore blocks which are about to expire from the cache
      if((((u_int)head->addr-(u_int)out)<<(32-TARGET_SIZE_2))>0x60000000+(MAX_OUTPUT_BLOCK_SIZE<<(32-TARGET_SIZE_2))) {
        if(verify_dirty(head->addr)) {
          //DebugMessage(M64MSG_VERBOSE, "restore candidate: %x (%d) d=%d",vaddr,page,invalid_code[vaddr>>12]);
  ...
  //DebugMessage(M64MSG_VERBOSE, "TRACE: count=%d next=%d (get_addr_32 no-match %x,flags %x)",g_cp0_regs[CP0_COUNT_REG],next_interrupt,vaddr,flags);
  int r=new_recompile_block(vaddr);
  if(r==0) return get_addr(vaddr);
  /* Execute in unmapped page, generate pagefault execption */
  return TLB_refill_exception_new(vaddr,vaddr&~1,0);
...
void invalidate_block(u_int block)
{
  ...
  while(head!=NULL) {
    ...
    if(vpage>2047||(head->vaddr>>12)==block) { // Ignore vaddr hash collision
      ...
      if(page<2048&&(signed int)start>=(signed int)0xC0000000&&(signed int)end>=(signed int)0xC0000000) {
        if(((start+memory_map[start>>12]-(u_int)g_dev.rdram.dram)>>12)<=page&&((end-1+memory_map[(end-1)>>12]-(u_int)g_dev.rdram.dram)>>12)>=page) {
          if((((start+memory_map[start>>12]-(u_int)g_dev.rdram.dram)>>12)&2047)<first) first=((start+memory_map[start>>12]-(u_int)g_dev.rdram.dram)>>12)&2047;
          if((((end-1+memory_map[(end-1)>>12]-(u_int)g_dev.rdram.dram)>>12)&2047)>last) last=((end-1+memory_map[(end-1)>>12]-(u_int)g_dev.rdram.dram)>>12)&2047;
        }
      }
    }
    head=head->next;
  ...
  if(block<0x100000&&tlb_LUT_w[block]) {
    ...
    // CHECK: Is this right?
    memory_map[block]=((tlb_LUT_w[block]&0xFFFFF000)-(block<<12)+(unsigned int)g_dev.rdram.dram-0x80000000)>>2;
    u_int real_block=tlb_LUT_w[block]>>12;
    invalid_code[real_block]=1;
    if(real_block>=0x80000&&real_block<0x80800) memory_map[real_block]=((u_int)g_dev.rdram.dram-0x80000000)>>2;
  }
  else if(block>=0x80000&&block<0x80800) memory_map[block]=((u_int)g_dev.rdram.dram-0x80000000)>>2;
  #ifdef USE_MINI_HT
...
void invalidate_all_pages(void)
{
  ...
  for(page=0;page<0x100000;page++) {
    if(tlb_LUT_r[page]) {
      memory_map[page]=((tlb_LUT_r[page]&0xFFFFF000)-(page<<12)+(unsigned int)g_dev.rdram.dram-0x80000000)>>2;
...
{
  //DebugMessage(M64MSG_VERBOSE, "TRACE: count=%d next=%d (checksum %x) lo=%8x%8x",g_cp0_regs[CP0_COUNT_REG],next_interrupt,mchecksum(),(int)(reg[LOREG]>>32),(int)reg[LOREG]);
  //DebugMessage(M64MSG_VERBOSE, "TRACE: count=%d next=%d (rchecksum %x)",g_cp0_regs[CP0_COUNT_REG],next_interrupt,rchecksum());
  //rlist();
  //if(tracedebug) {
  ...
  //if(0) {
    DebugMessage(M64MSG_VERBOSE, "TRACE: count=%d next=%d (checksum %x)",g_cp0_regs[CP0_COUNT_REG],next_interrupt,mchecksum());
    //DebugMessage(M64MSG_VERBOSE, "TRACE: count=%d next=%d (checksum %x) Status=%x",g_cp0_regs[CP0_COUNT_REG],next_interrupt,mchecksum(),g_cp0_regs[CP0_STATUS_REG]);
    //DebugMessage(M64MSG_VERBOSE, "TRACE: count=%d next=%d (checksum %x) hi=%8x%8x",g_cp0_regs[CP0_COUNT_REG],next_interrupt,mchecksum(),(int)(reg[HIREG]>>32),(int)reg[HIREG]);
    rlist();
    #if NEW_DYNAREC == NEW_DYNAREC_X86
...
static void do_ccstub(int n)
{
  ...
  emit_readword((int)&next_interrupt,EAX);
...
static void ujump_assemble(int i,struct regstat *i_regs)
{
  ...
  /* Handle a ROM-hack pattern where a JAL's delay slot overwrites
   * the link register $31 itself (e.g. ORI $31, $31, imm). Without
   * the early link-register write, our normal post-delay-slot
   * link-write at the bottom of this function would be clobbered
   * by the delay slot's own store, and the called subroutine would
   * never see the right return address. Emit the link-register
   * write here, before ds_assemble runs the delay slot, when we
   * can see that rt1[i+1]==31. The matching post-ds path below
   * skips its own link-write in that case. */
  if(rt1[i+1]==31)
  {
    assert(rs1[i+1]==31);
    signed char rt=get_reg(branch_regs[i].regmap,31);
    assert(get_reg(i_regs->regmap,31)==rt);
    if(rt>=0) emit_movimm(start+i*4+8,rt);
  }
...
//DEBUG >
  ...
  emit_readword((int)&last_count,ECX);
  emit_add(HOST_CCREG,ECX,HOST_CCREG);
  emit_readword((int)&next_interrupt,ECX);
  emit_writeword(HOST_CCREG,(int)&g_cp0_regs[CP0_COUNT_REG]);
...
void new_dynarec_init(void)
{
  ...
  int ret = sceKernelGetMemBlockBase(sceBlock, (void **)&base_addr);
  if (ret < 0)
  ...
  for(n=524288;n<526336;n++) // 0x80000000 .. 0x807FFFFF
    memory_map[n]=((u_int)g_dev.rdram.dram-0x80000000)>>2;
...
*/
  ...
  //DebugMessage(M64MSG_VERBOSE, "TRACE: count=%d next=%d (compile %x)",g_cp0_regs[CP0_COUNT_REG],next_interrupt,addr);
  //if(debug)
  //DebugMessage(M64MSG_VERBOSE, "TRACE: count=%d next=%d (checksum %x)",g_cp0_regs[CP0_COUNT_REG],next_interrupt,mchecksum());
  ...
  if ((int)addr >= 0xa0000000 && (int)addr < 0xa07fffff) {
    source = (u_int *)((u_int)g_dev.rdram.dram+start-0xa0000000);
    ...
    //if(tlb_LUT_r[start>>12])
      //source = (u_int *)(((int)g_dev.rdram.dram)+(tlb_LUT_r[start>>12]&0xFFFFF000)+(((int)addr)&0xFFF)-0x80000000);
  ...
  for(i=0;i<slen;i++)
  {
     ...
     * the called code reads a stale upper half. Saving the
     * regmap before the optim runs preserves the eviction's
     * effect locally (current.regmap still loses the upper half
     * for the rest of this instruction's processing) without
     * corrupting the writeback bookkeeping. */
...
//    if((void*)assem_debug==(void*)printf) disassemble_inst(i);
  ...
  for(i=start>>12;i<=(int)((start+slen*4)>>12);i++) {
    ...
    if((signed int)start>=(signed int)0xC0000000) {
      assert(using_tlb);
      j=(((u_int)i<<12)+(memory_map[i]<<2)-(u_int)g_dev.rdram.dram+(u_int)0x80000000)>>12;
      invalid_code[j]=0;
...
void TLBWI_new(void)
{
  ...
  for (i=tlb_e[g_cp0_regs[CP0_INDEX_REG]&0x3F].start_even>>12; i<=tlb_e[g_cp0_regs[CP0_INDEX_REG]&0x3F].end_even>>12; i++)
  {
    ...
    {
      if(tlb_LUT_r[i]) {
        memory_map[i]=((tlb_LUT_r[i]&0xFFFFF000)-(i<<12)+(unsigned int)g_dev.rdram.dram-0x80000000)>>2;
