...
%endif
    ...
    push rbp
    add     rsp,    -56
    ;Resume at pcaddr (seeded to the boot vector by new_dynarec_init), so
    ;new_dyna_start is re-enterable per frame. get_addr_ht compiles the block
    ;if needed and returns its host (RX) address.
    mov     ARG1_REG,    DWORD [rel g_dev_r4300_new_dynarec_hot_state_pcaddr]
    call    get_addr_ht
    mov     CCREG,    DWORD [rel g_dev_r4300_new_dynarec_hot_state_cycle_count]
    jmp     rax
