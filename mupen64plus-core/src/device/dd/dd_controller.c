...
/* ----------------------------------------------------------------------------
 * region 13: bridge mupen64plus-next's r4300/interrupt API (used by this
 * imported dd_controller) onto parallel-n64's equivalents. next threads a
 * struct r4300_core and cp0 through these calls; pn64 uses file-global cp0 state,
 * so the extra arguments are accepted and ignored here.
 * ------------------------------------------------------------------------- */

/* DD address-space region bases (next defines these in device.h; kept local
 * to the DD subsystem here to avoid touching parallel-n64's device.h). */
#ifndef MM_DD_C2S_BUFFER
...
/* R4300 segment bases used for cached-code invalidation. */
#ifndef R4300_KSEG0
#define R4300_KSEG0 UINT32_C(0x80000000)
#define R4300_KSEG1 UINT32_C(0xa0000000)
#endif

static osal_inline void dd_check_interrupt(struct r4300_core* r4300, uint32_t cause, int set)
{
    (void)r4300; (void)cause;
    if (set)
        g_cp0_regs[CP0_CAUSE_REG] |= CP0_CAUSE_IP3;
    else
        g_cp0_regs[CP0_CAUSE_REG] &= ~CP0_CAUSE_IP3;
    check_interrupt();
}
#define r4300_check_interrupt(r4300, cause, set) dd_check_interrupt((r4300), (cause), (set))

/* next: add_interrupt_event(&cp0, type, cycles); pn64: add_interrupt_event(type, delay) */
#define dd_add_interrupt_event(cp0ptr, type, cycles) add_interrupt_event((type), (cycles))
#define add_interrupt_event(cp0ptr, type, cycles)    dd_add_interrupt_event((cp0ptr), (type), (cycles))

/* next: remove_event(&cp0.q, type); pn64: remove_event(type) */
#define dd_remove_event(qptr, type) remove_event((type))
#define remove_event(qptr, type)    dd_remove_event((qptr), (type))

/* next: cp0_update_count(r4300); pn64: cp0_update_count(void) */
#define dd_cp0_update_count(r4300) cp0_update_count()
#define cp0_update_count(r4300)    dd_cp0_update_count((r4300))

/* next: invalidate_r4300_cached_code(r4300, addr, len); pn64: (addr, len) */
#define dd_invalidate_cc(r4300, addr, len) invalidate_r4300_cached_code((addr), (len))
#define invalidate_r4300_cached_code(r4300, addr, len) dd_invalidate_cc((r4300), (addr), (len))
...
int read_dd_rom(void* opaque, uint32_t address, uint32_t* value)
{
    struct dd_controller* dd = (struct dd_controller*)opaque;
...
int write_dd_rom(void* opaque, uint32_t address, uint32_t value, uint32_t mask)
{
