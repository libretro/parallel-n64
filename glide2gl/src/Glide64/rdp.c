...
static void rdp_fillrect(uint32_t w0, uint32_t w1)
{
   ...
   pd_multiplayer = (settings.ucode == ucode_PerfectDark) && (rdp.cycle_mode == 3) && (rdp.fill_color == 0xFFFCFFFC);
...
EXPORT void CALL ProcessRDPList(void)
{
   ...
   while (rdp_cmd_cur < rdp_cmd_ptr)
   {
      ...
      switch(cmd)
      {
         ...
         case 0x37:
            // 0x37, Set_Fill_Color
            //rdp_setfillcolor(w0, w1);
            rdp.fill_color = w1;
            rdp.update |= UPDATE_ALPHA_COMPARE | UPDATE_COMBINE;
            break;
         case 0x38:
            // 0x38, Set_Fog_Color
            //rdp_setfogcolor(w0, w1);
            rdp.fog_color = w1;
            rdp.update |= UPDATE_COMBINE | UPDATE_FOG_ENABLED;
            break;
         case 0x39:
            // 0x39, Set_Blend_Color
            //rdp_setblendcolor(w0, w1);
            rdp.blend_color = w1;
            rdp.update |= UPDATE_COMBINE;
            break;
         case 0x3a:
            //0x3a, Set_Prim_Color
            //rdp_setprimcolor(w0, w1);
            rdp.prim_color = w1;
            rdp.prim_lodmin = (w0 >> 8) & 0xFF;
            rdp.prim_lodfrac = max(w0 & 0xFF, rdp.prim_lodmin);
            rdp.update |= UPDATE_COMBINE;
         ...
         case 0x3c:
            ...
            break;
         case 0x3d:
            rdp_settextureimage(w0, w1);
            break;
         case 0x3e:
            //0x3e, Set_Mask_Image
            //rdp_setdepthimage(w0, w1);
