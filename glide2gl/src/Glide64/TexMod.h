...
static void mod_tex_inter_color_using_factor (uint16_t *dst, int size, uint32_t color, uint32_t factor)
{
   ...
   do
   {
      uint8_t r = (percent_i * (((*dst) >> 8) & 0xF) + percent * cr);
      uint8_t g = (percent_i * (((*dst) >> 4) & 0xF) + percent * cg);
      uint8_t b = (percent_i * ((*dst) & 0xF) + percent * cb);
...
static void mod_full_color_sub_tex (uint16_t *dst, int size, uint32_t color)
{
   do
   {
      uint8_t a = ((color & 0xF) - (((*dst) >> 12) & 0xF));
      uint8_t r = (((color >> 12) & 0xF) - (((*dst) >> 8) & 0xF));
      uint8_t g = (((color >> 8) & 0xF) - (((*dst) >> 4) & 0xF));
      uint8_t b = (((color >> 4) & 0xF) - ((*dst) & 0xF));
      *(dst++) = (a << 12) | (r << 8) | (g << 4) | b;
   }while(--size);
}
...
static void mod_col_inter_col1_using_texa (uint16_t *dst, int size, uint32_t color0, uint32_t color1)
{
   ...
   do
   {
      ...
      uint8_t r = (percent_i * cr0 + percent * cr1);
      uint8_t g = (percent_i * cg0 + percent * cg1);
      uint8_t b = (percent_i * cb0 + percent * cb1);
      *(dst++) = (*dst & 0xF000) | (r << 8) | (g << 4) | b;
   }while(--size);
}
...
static void mod_col_inter_col1_using_texa__mul_tex (uint16_t *dst, int size, uint32_t color0, uint32_t color1)
{
   ...
   do
   {
      float percent = ((*dst & 0xF000) >> 12) / 15.0f;
      float percent_i = 1.0f - percent;
      uint8_t r = (((percent_i * cr0 + percent * cr1) / 15.0f) * ((((*dst) & 0x0F00) >> 8) / 15.0f) * 15.0f);
      uint8_t g = (((percent_i * cg0 + percent * cg1) / 15.0f) * ((((*dst) & 0x00F0) >> 4) / 15.0f) * 15.0f);
      uint8_t b = (((percent_i * cb0 + percent * cb1) / 15.0f) * (((*dst) & 0x000F) / 15.0f) * 15.0f);
      *(dst++) = (*dst & 0xF000) | (r << 8) | (g << 4) | b;
   }while(--size);
}
...
static void mod_col_inter_tex_using_tex (uint16_t *dst, int size, uint32_t color)
{
   ...
   do
   {
      ...
      float percent_g = (((*dst) >> 4) & 0xF) / 15.0f;
      float percent_b = ((*dst) & 0xF) / 15.0f;
      uint8_t r = ((1.0f-percent_r) * cr + percent_r * (((*dst) & 0x0F00) >> 8));
      uint8_t g = ((1.0f-percent_g) * cg + percent_g * (((*dst) & 0x00F0) >> 4));
      uint8_t b = ((1.0f-percent_b) * cb + percent_b * ((*dst) & 0x000F));       
      *(dst++) = (*dst & 0xF000) | (r << 8) | (g << 4) | b;
   }while(--size);
}
   ...
   do
	{
		float percent = ((*dst & 0xF000) >> 12) / 15.0f;
		float percent_i = 1.0f - percent;
		uint8_t r = (percent_i * cr + percent * ((*dst & 0x0F00) >> 8));
		uint8_t g = (percent_i * cg + percent * ((*dst & 0x00F0) >> 4));
		uint8_t b = (percent_i * cb + percent * (*dst & 0x000F));
		*(dst++) = (*dst & 0xF000) | (r << 8) | (g << 4) | b;
	}while(--size);
}
...
{
	float base_a_plus_percent = ((1.0f - (factor / 255.0f)) * 15.0f) + (factor / 255.0f);
...
static void mod_tex_sub_col (uint16_t *dst, int size, uint32_t color)
{
   ...
   do
   {
      uint8_t r = max(((((*dst) >> 8) & 0xF) - cr), 0);
      uint8_t g = max(((((*dst) >> 4) & 0xF) - cg), 0);
      uint8_t b = max((((*dst) & 0xF) - cb), 0);
